...
for site, sport, url in zip(sites, sports, urls):
    ...
    soup = BeautifulSoup(page_source)
#     team_names = [tag.text for tag in soup.find_all(class_=["col_teamname bdevtt"])]
#     money_lines = [tag.text for tag in soup.find_all(class_=["odds bdevtt moneylineodds displayOdds"])]
